...
const onSubmit = async (
	...
	edittingId: string,
	setEdittingId: Dispatch<SetStateAction<string | null>>,
	toast: ToastInvoker
) => {
	const iamName = encodeIamName(formData.name);
	const updatedRole = { ...formData, iamName };
	try {
		toast({ title: `Updating role "${formData.name}"` });
		const resp = await fetch(`/api/roles/${edittingId}`, {
			method: "PATCH",
...
export const EditRoleForm: FC<Props> = (props) => {
	const { toast } = useToast();
	const form = useForm<FormSchema>({
		resolver: zodResolver(FormSchema),
		defaultValues: async () => {
			const resp = await fetch(`/api/roles/${props.editting}`);
			const body = await resp.json();
			const currentRole = Role0Schema.parse(body);
			return {
				...currentRole,
				views: currentRole.views ?? [],
