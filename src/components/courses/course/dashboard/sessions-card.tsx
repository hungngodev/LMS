...
import { useAllCourseSessionsCount } from "@/hooks/sessions/use-all-course-sessions-count";
...
const useViews = (user: User2Schema | null) => {
	...
	useEffect(() => {
		...
		setShowCount(views.has(ViewEnum["courses:dashboard:sessions:count"]));
		setShowCreate(views.has(ViewEnum["courses:dashboard:sessions:create"]));
	}, [user]);
...
export const SessionsCard: FC<Props> = (props) => {
	const { user } = useAuth();
	const { showCount, showCreate } = useViews(user);

	const SessionsCount: FC = () => {
		const { count } = useAllCourseSessionsCount(props.courseId);
		if (count !== undefined) return <Header4>{count}</Header4>;
		else return <Skeleton className="h-12 w-32 rounded-lg" />;
	};
	const SessionsView: FC = () => {
		return (
			<Link href={`/courses/${props.courseId}/sessions`}>
				<Button className="w-full">View</Button>
			</Link>
		);
	};
	const SessionsCreate: FC = () => {
	...
	return (
		<Card>
			...
			<CardContent className="flex flex-col space-y-4 w-full">
				{showCount && <SessionsCount />}
				{showCount && <SessionsView />}
				{showCreate && <SessionsCreate />}
			</CardContent>
		</Card>
	);
};
