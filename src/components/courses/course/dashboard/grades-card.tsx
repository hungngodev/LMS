...
import { useAllCourseGradesCount } from "@/hooks/grades/use-all-course-grades-count";
...
const useViews = (user: User2Schema | null) => {
	...
	useEffect(() => {
		...
		setShowCount(views.has(ViewEnum["courses:dashboard:grades:count"]));
		setShowCreate(views.has(ViewEnum["courses:dashboard:grades:create"]));
	}, [user]);
	...
	return (
		<Card>
			...
			<CardContent className="flex flex-col space-y-4 w-full">
				{showCount && <GradesCount />}
				{showCount && <GradesView />}
				{showCreate && <GradesCreate />}
			</CardContent>
		</Card>
	);
};
