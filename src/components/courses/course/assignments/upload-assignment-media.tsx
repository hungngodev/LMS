...
const useDocuments = (documentsId: string[]) => {
	...
	const endpoint = `/api/assignmentsMedia${qs.stringify(
...
export interface UploadAssignmentMediaProps {
	mode: "create" | "edit";
	courseId: string;
	documentsId: string[];
...
export const UploadAssignmentMedia: FC<UploadAssignmentMediaProps> = ({
	mode,
	courseId,
	documentsId,
	setValue
	...
	const onDrop = useCallback(
		async (acceptedFiles: File[]) => {
			const newDocumentsId: string[] = await Promise.all(
				acceptedFiles.map(async (file) => {
					...
					const resp = await fetch("/api/assignmentsMedia", {
						method: "POST",
						body: formData
					});
					toast({
		...
		[courseId, documentsId, setValue, toast]
	);
			...
			if (mode === "create") {
				...
				try {
					const resp = await fetch(
						`/api/assignmentsMedia/${documentsId[fileIndex]}`,
						{
							method: "DELETE",
							headers: {
