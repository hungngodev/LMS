...
/**
 * fetch all count of past assignments of a given course
 */
export const usePastCourseAssignmentsCount = (courseId: string) => {
	const query = useMemo<Where>(() => {
		return {
			and: [
				{ course: { equals: courseId } },
				{ dueAt: { less_than: new Date().toISOString() } } as Where
			]
		};
	}, [courseId]);
