...
const canCreateSubmissionsMedia: Access<
	SubmissionMedia0Schema,
	User2Schema
...
const canReadSubmissionsMedia: Access<SubmissionMedia0Schema, User2Schema> = ({
	req: { user }
}) => {
...
const canUpdateSubmissionsMedia: Access<
	SubmissionMedia0Schema,
	User2Schema
> = ({ req: { user } }) => {
	if (!user) return false;
...
const canDeleteSubmissionsMedia: Access<
	SubmissionMedia0Schema,
	User2Schema
> = ({ req: { user } }) => {
	if (!user) return false;
...
export const access: CollectionConfig["access"] = {
	create: canCreateSubmissionsMedia,
	read: canReadSubmissionsMedia,
	update: canUpdateSubmissionsMedia,
	delete: canDeleteSubmissionsMedia
};
