...
const canCreateSession: Access<SessionRecurrence0Schema, User2Schema> = ({
	req: { user }
}) => {
...
const canReadSession: Access<SessionRecurrence0Schema, User2Schema> = ({
	req: { user }
}) => {
	if (!user) return false;
	const actions = getUserAllowedActions(user);
...
const canUpdateSession: Access<SessionRecurrence0Schema, User2Schema> = ({
	req: { user }
}) => {
	if (!user) return false;
	const actions = getUserAllowedActions(user);
...
const canDeleteSession: Access<SessionRecurrence0Schema, User2Schema> = ({
	req: { user }
}) => {
	if (!user) return false;
	const actions = getUserAllowedActions(user);
