...
const canCreateAssignmentsMedia: Access<
...
const canReadAssignmentsMedia: Access<Assignment0Schema, User2Schema> = ({
	req: { user }
}) => {
...
const canUpdateAssignmentsMedia: Access<Assignment0Schema, User2Schema> = ({
	req: { user }
}) => {
	if (!user) return false;
	const actions = getUserAllowedActions(user);
...
const canDeleteAssignmentsMedia: Access<Assignment0Schema, User2Schema> = ({
	req: { user }
}) => {
	if (!user) return false;
	const actions = getUserAllowedActions(user);
...
export const access: CollectionConfig["access"] = {
	create: canCreateAssignmentsMedia,
	read: canReadAssignmentsMedia,
	update: canUpdateAssignmentsMedia,
	delete: canDeleteAssignmentsMedia
};
